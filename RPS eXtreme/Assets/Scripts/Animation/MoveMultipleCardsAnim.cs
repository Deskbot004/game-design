...
public class MoveMultipleCardsAnim : MoveCardAnim
{
    ...
    public List<Vector3> targetLocalRotations = new(); // TODO: Change to World
    ...
    protected override IEnumerator PlaySpecificAnimation() {
        ...
        for (int i=0; i<cards.Count; i++) {
            cardsInMotion.Add(StartCoroutine(MoveCard(cards[i], targetWorldPositions[i], Quaternion.Euler(targetLocalRotations[i]))));
    ...
    public bool AreThereEnoughTargets() {
        return cards.Count == targetWorldPositions.Count || cards.Count == targetLocalRotations.Count;
    }
}